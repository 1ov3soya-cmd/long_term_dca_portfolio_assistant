...
class RunArchiver:
    ...
    def start_run(self, command_name: str) -> dict[str, Any] | None:
        ...
        run_id = f"{started_at:%Y%m%d_%H%M%S_%f}_{command_name.replace('-', '_')}"
    ...
    def finalize(
        ...
        normalized_artifacts = self._normalize_artifacts(output_artifacts)
        copied_artifacts = self._copy_artifacts_if_enabled(run_dir, normalized_artifacts)
        ...
        self._write_json(
            paths["output_artifacts"],
            {
                "original_outputs": normalized_artifacts,
                "copied_outputs": copied_artifacts,
            },
        )
    ...
    def _copy_artifacts_if_enabled(
        self,
        run_dir: Path,
        output_artifacts: dict[str, Any],
    ) -> dict[str, str]:
        """按配置选择性复制关键输出文件，第一版默认关闭。"""

        if not bool(self.settings.get("archive_copy_reports", False)):
            return {}
        ...
        for key, value in output_artifacts.items():
            if isinstance(value, dict):
                for sub_key, sub_value in value.items():
                    copied.update(self._copy_single_artifact(artifacts_dir, f"{key}.{sub_key}", sub_value))
            elif isinstance(value, list):
                for index, item in enumerate(value):
                    copied.update(self._copy_single_artifact(artifacts_dir, f"{key}.{index}", item))
            else:
                copied.update(self._copy_single_artifact(artifacts_dir, key, value))
        return copied
    ...
    def _copy_single_artifact(
        ...
        safe_name = artifact_key.replace("\\", "_").replace("/", "_")
        destination = artifacts_dir / f"{safe_name}{source.suffix}"
        shutil.copy2(source, destination)
        return {artifact_key: str(destination)}

    def _code_version_hint(self) -> str:
        """尽量记录当前 git commit 简写，不保证始终可用。"""
